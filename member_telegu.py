...
def convert_to_telugu(op):
    llm = OpenAI(temperature=0)
    prompt = PromptTemplate(
        input_variables=["result", "language"],
        template="Translate the {result} in {language}",
        )
...
def member_page():
    ...
    ### OpenAI API Key
    # load_dotenv() # for hosting on local machine
    # api = os.getenv("api_key")
    #print(api)
    api = st.secrets["openai_api_key"] # for hosting on streamlit cloud
    #api="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"
    os.environ["OPENAI_API_KEY"] = api

    question=""
    # input question
    ask_text = st.text_input("Ask a Question")
    col1, col4,col2,  col3 = st.columns([2, 1,2, 2])
    with col1:
    ...
    with col2:
        ...
        audio_bytes = audio_recorder(text = "Speak", icon_size="2x", key="audio_button")
